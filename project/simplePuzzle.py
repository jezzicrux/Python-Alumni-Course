...
class Death(Scene):
	quips = [
		...
		"You were blown up by the Holy Hand Grenade of Antioch"
		]
	def enter(self):
	...
	def enter(self):
		...
		if action == str.lower("spam"):
			...
			if second_choice == str.lower("spam"):
				print("Alright! Stop That! It's gotten much too silly!")
				print("")
				return 'death'
			elif second_choice in spamsuper:
			...
			print("You don't like spam! BEGONE WITH YOU!!")
			print("")
			return 'death'
		elif action in spamsuper:
			print(dedent("""
			We're all out of that. Why don't you just go the parrot room?
			You are pretty much forced in a low lit room with nothing in it
			but a bird cage. You walk up to the bird cage and see a blue parrot."""))
			print("")
			return 'the_Parrot_Room'
...
class ParrotRoom(Scene):
	...
	Randomness = [
        ...
        """[Parrot] Questions? Questions? No point in talking to me, I'm just a parrot!""",
	...
	def enter(self):
		...
		if action == str.lower("take the parrot"):
	        ...
	        [Parrot] Ahah! Not fast enough! Squawk!
	        ...
	        [Parrot] Oi! Stop it! Stop it! I'm standing on that! Leave it alone!
	                 You are an incredibly rube person! I'll peck you! I'll screech!
...
class TheBridgeofDoom(Scene):
	...
	def enter(self):
		...
		if action == str.lower("yes"):
			...
			final_answer = input(">> ")
			if final_chioce == "[Bridgekeeper] What... is the capital of Assyria?":
